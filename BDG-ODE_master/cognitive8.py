...
#建立邻接矩阵A
file_path = 'f5/f5\edge_run50000.npy'
...
#     plt.figure()
#     # 画出负面感染比例
#     plt.plot(negative_infection_ratio_list, label='Negative Infection Ratio')
#     # 画出正面感染比例
#     plt.plot(positive_infection_ratio_list, label='Positive Infection Ratio')
#     # 添加图例
#     plt.legend()
#     # 显示图像
#     plt.show()

# calculate_infection_ratio(A, true_y)
...
if args.baseline == 'ndcn':
    ...
    p_hidden = 10
    prep_hidden = 10
    model = NDCN2(input_size=input_size, hidden_size=hidden_size, A=OM, num_classes=num_classes, n=n,
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % 5 == 0:
            with torch.no_grad():
                ...
                # alter_users_true1 = [(x/(x + y)) * 9484 for x, y in zip(pos_users_true, neg_users_true)]
                # alter_users_true2 = [(y/(x + y)) * 9484 for x, y in zip(pos_users_true, neg_users_true)]
                # alter_users_pred1 = [(x/(x + y)) * 9484 for x, y in zip(pos_users_pred, neg_users_pred)]
                # alter_users_pred2 = [(y/(x + y)) * 9484 for x, y in zip(pos_users_pred, neg_users_pred)]                
                plt.figure()
...
# python cognitive8.py --T 5 --sampled_time irregular --baseline ndcn --gpu 1 --weight_decay 1e-3 --lr 0.01 --sparse --niters 1500 --dump